...
module.exports = {
    ...
    async getLyric(musicItem) {
        ...
        if (!res.data || !res.data.lrc) return { rawLrc: '' };
        try {
            const lrcRes = await axios.get(res.data.lrc, { headers: { 'Referer': BASE_URL } });
            return { rawLrc: typeof lrcRes.data === 'string' ? lrcRes.data : '' };
        } catch (e) {
            return { rawLrc: '' };
        }
    },
